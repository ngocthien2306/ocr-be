...
def draw_ocr2(image, boxes, txts=None, scores=None, drop_score=0.5, box_color=(255, 0, 0), text_color=(0, 255, 0), text_size=1):
    """
    Visualize the results of OCR detection and recognition
    args:
        ...
        boxes (list): boxes with shape (N, 4, 2)
        txts (list): the texts
        scores (list): txts corresponding scores
        drop_score (float): only scores greater than drop_threshold will be visualized
        box_color (tuple): color of the box in BGR format
    ...
    for i in range(box_num):
        ...
        # Convert box points to a format suitable for cv2.rectangle
        p1 = (int(box[0][0]), int(box[0][1]))
        p2 = (int(box[2][0]), int(box[2][1]))

        # Draw the bounding box
        lw = int(round(0.0015 * (image.shape[0] + image.shape[1]) / 2)) + 1  # line thickness
        cv2.rectangle(image, p1, p2, box_color, thickness=lw, lineType=cv2.LINE_AA)

        if txts is not None:
            # text = f"{txts[i]} ({score:.2f})"
            text = f"{txts[i]}"
            tf = max(lw - 1, 1)  # font thickness

            # Get text size
            (w, h), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, text_size, tf)
            
            # Adjust text position if it goes out of the image
            text_origin = (p1[0], p1[1] - 2)
            if text_origin[1] - h < 0:
                text_origin = (p1[0], p1[1] + h + 2)
            
            # Background rectangle for text
            p2_text = (text_origin[0] + w, text_origin[1] - h - 3) if text_origin[1] - h >= 3 else (text_origin[0] + w, text_origin[1] + h + 3)
            cv2.rectangle(image, text_origin, p2_text, box_color, -1, cv2.LINE_AA)
            
            # Put the text
            cv2.putText(image, text, (text_origin[0], text_origin[1] - 2 if text_origin[1] - h >= 3 else text_origin[1] + h + 2), cv2.FONT_HERSHEY_SIMPLEX, text_size, text_color, tf, lineType=cv2.LINE_AA)
